...
import main.java.analysis.Metric;
...
public class TestAnalysisProcessorCsv {
	...
	@Test(expected=NullPointerException.class) //temp
	public void testApply() throws IOException {
		Collection<WebsiteResult> websites = new ArrayList<WebsiteResult>();
		websites.add(mock(WebsiteResult.class));
		StateAnalysisMetric stateMetric = new StateAnalysisMetric(mock(DatabaseUtils.class), mock(NearDuplicateDetection.class));
		SpeedMetric speedMetric = new SpeedMetric();
		ImmutableList<Metric> metrics = new ImmutableList.Builder<Metric>().add(stateMetric).add(speedMetric).build();
	...
	public void testApplyInvalidMetrics() throws IOException {
		Collection<WebsiteResult> websites = new ArrayList<WebsiteResult>();
		websites.add(mock(WebsiteResult.class));
		Analysis analysis = new Analysis("Test-analysis-invalid-metrics", websites, new ImmutableList.Builder<Metric>().build());
